...
class DMBP(nn.Module):
    """Dense multi-branch perception with parallel receptive fields."""

    def __init__(self, in_channels, out_channels):
        super(DMBP, self).__init__()
    ...
    def forward(self, x):
        ...
        for branch in self.branches:
            feature = branch(running)
            dense_features.append(feature)
            if feature.shape[1] == x.shape[1]:
                running = running + feature
        return self.fuse(torch.cat(dense_features, dim=1))


class BLFD(nn.Module):
    """Bidirectional decoder for semantic propagation and detail recovery."""

    def __init__(self, in_channels, out_channels=64):
        super(BLFD, self).__init__()
    ...
    def forward(self, features):
        if len(features) != len(self.projections):
            raise ValueError("The number of features must match in_channels.")

        projected = [layer(feature)
                     for layer, feature in zip(self.projections, features)]

        top_down = list(projected)
        for index in range(len(top_down) - 2, -1, -1):
            semantic = F.interpolate(
                top_down[index + 1], size=top_down[index].shape[-2:],
                mode='bilinear', align_corners=False)
            top_down[index] = self.refine[index](top_down[index] + semantic)

        bottom_up = list(top_down)
        for index in range(1, len(bottom_up)):
            detail = F.adaptive_avg_pool2d(
                bottom_up[index - 1], bottom_up[index].shape[-2:])
            bottom_up[index] = self.refine[index](bottom_up[index] + detail)
        return bottom_up


class MSAF(nn.Module):
    """Adaptive attention fusion for multi-scale edge predictions."""

    def __init__(self, in_channels=6, hidden_channels=16,
                 region_scales=(1, 4, 8, 16)):
    ...
    def forward(self, predictions):
        x = (torch.cat(predictions, dim=1)
             if isinstance(predictions, (list, tuple)) else predictions)
        if x.shape[1] != self.in_channels:
            raise ValueError(
                "Expected %d prediction channels, got %d."
                % (self.in_channels, x.shape[1]))

        regional = 0
        for scale in self.region_scales:
            pooled = F.adaptive_avg_pool2d(x, (scale, scale))
            attended = self.region_attention(pooled)
            regional = regional + F.interpolate(
                attended, size=x.shape[-2:], mode='bilinear',
                align_corners=False)

        weights = torch.sigmoid(self.pixel_attention(x) + regional)
